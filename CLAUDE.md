...
- **Routing** (`src/App.tsx`): `react-router-dom` v7 `BrowserRouter`. All routes are `lazy()`-loaded under a single `Layout` route (`/` → Home, `/experience`, `/blog`, `/blog/:slug`), wrapped in `Suspense`. `ScrollToTop` resets scroll on navigation. Sitemap routes are served as prerendered static HTML; unknown paths fall back to the SPA shell via `public/_redirects` (`/* /index.html 200`) and hydrate to the `noindex` 404.
